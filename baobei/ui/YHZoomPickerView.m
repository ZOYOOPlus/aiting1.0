...
#import "YHZoomPickerView.h"
#import "YHStaticTextCell.h"

@interface YHZoomPickerView ()<UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate>
@property (nonatomic, strong) UITableView *tableView;
@end


@implementation YHZoomPickerView
#pragma mark - 初始化
...
- (void)refreshPickerView:(CGFloat)rowOffset
{
    ...
    {
        NSUInteger rowIndex = row.integerValue;
        UITableViewCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:rowIndex inSection:0]];
        ...
        else if (rowIndex == currentRow - 1)
        {
            // 当前选中行的上一行
            if (rowOffset >= 0)
                cell.transform = CGAffineTransformMakeScale(_unselectedRowScale, _unselectedRowScale);
            else
            {
                CGFloat delta = ( -rowOffset) / _rowHeight;
                CGFloat deltaScale = _unselectedRowScale + (1 - _unselectedRowScale) * delta;
                cell.transform = CGAffineTransformMakeScale(deltaScale, deltaScale);
            }
        }
        else if (rowIndex == currentRow + 1)
...
{
    static NSString *cellIdentifier = @"CLZoomPickerViewCellID";
    
    // 创建cell
    YHStaticTextCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil)
    {
        cell = [[YHStaticTextCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
