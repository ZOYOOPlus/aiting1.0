...
/*
 ...
 宽容：如果宽容是0比它会搜索色彩精确匹配
                  其他明智它会根据公差值取范围。
             
                  如果你不想用宽容和希望incress性能比你可以改变
                  compareColor（ocolor，颜色，公差）只有ocolor==颜色，reduse函数调用。
*/
...
- (UIImage *) floodFillFromPoint:(CGPoint)startPoint withColor:(UIColor *)newColor andTolerance:(int)tolerance useAntiAlias:(BOOL)antiAlias
{
    @try
    {
        /*
         ...
         首先，我们创建一个从UIImage的行数据。我们要求这次谈话，使我们可以在像素像素使用细节喜欢的颜色。你可以得到关于这个话题在这里一些讨论：
        ...
        */
        ...
        unsigned char *imageData = malloc(height * width * 6);
